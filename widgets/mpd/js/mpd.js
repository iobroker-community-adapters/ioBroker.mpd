...
// this code can be placed directly in mpd.html
vis.binds.mpd = {
    version: "0.5.0",
    showVersion: function () {
        if (vis.binds.mpd.version) {
            console.log('Version mpd: ' + vis.binds.mpd.version);
            vis.binds.mpd.version = null;
        }
    },
	createWidget: function (widgetID, view, data, style) {
        ...
        if (!$div.length) {
            return setTimeout(function () {
                vis.binds.mpd.createWidget(widgetID, view, data, style);
            }, 100);
        }
        ...
        if (data.oid) {
            vis.states.bind(data.oid + '.val', function (e, newVal, oldVal) {
                $div.find('.mpd-value').html(newVal);
            });
        }
    }
};
	
vis.binds.mpd.showVersion();
