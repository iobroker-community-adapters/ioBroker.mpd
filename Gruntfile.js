...
module.exports = function (grunt) {
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new mpd name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/mpd.png')) {
            fs.renameSync(__dirname + '/admin/mpd.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/mpd.html')) {
            fs.renameSync(__dirname + '/widgets/mpd.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/mpd/js/mpd.js')) {
            fs.renameSync(__dirname + '/widgets/mpd/js/mpd.js', __dirname + '/widgets/mpd/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/mpd')) {
            fs.renameSync(__dirname + '/widgets/mpd',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Mpd/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Mpd'
                        },
                        {
                            match: /instalator/g,
                            replacement: author
